  ...
  const commands = {
    'play': (message) => {
      if (args.length != 0) {
        ...
        if (!message.guild.voiceConnection) return commands.join(message).then(() => {
        ...
        if (queue[message.guild.id] === undefined) {
          return message.channel.send({
            embed: {
              color: 15844367,
              description: `Add some songs to the queue first with ${config.serverconfigs[message.guild.id].prefix}play`
            }
          });
        }
        ...
        'add': async (message) => {
            ...
            if (url == '' || url === undefined) {
              ...
              yt.getInfo(url, async (err, info) => {
                ...
                if (queue[message.guild.id].songs.length === 1 && queue[message.guild.id].playing === false) {
